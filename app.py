        ...
        if response and response.text:
            ...
            try:
                ...
                if isinstance(raw_json, dict):
                    parsed_dict = normalize_parsed_keys(raw_json)
            except Exception:
                pass
            
            # 2차 시도: 누락된 항목을 정규식으로 보강
            regex_dict = extract_values_from_text(raw_text)
            ...
            return None, "AI 모델로부터 응답을 받지 못했습니다."

    except Exception as e:
        return None, f"이미지 분석 오류: {str(e)}"
...
# --------------------------------------------------------------------------
# TAB 1: 실험 데이터 입력 및 기초 M/B 연산 (위젯 강제 갱신 엔진 적용)
# --------------------------------------------------------------------------
with main_tab1:
    v_ver = st.session_state.widget_ver

    with st.expander("📷 [AI Vision] 수기 실험 노트/기록지 사진으로 자동 입력 (클릭하여 열기)", expanded=True):
        col_img1, col_img2 = st.columns([2, 1])
        with col_img1:
            uploaded_note_img = st.file_uploader(
                "수기 또는 타이핑 인쇄물 실험 일지 사진 업로드 (JPG, PNG)", 
                type=["jpg", "jpeg", "png"],
                key=f"up_note_img_{v_ver}"
            )
        with col_img2:
            st.write("")
            st.write("")
                    ...
                    with st.spinner("Gemini AI가 일지 내용을 정밀 판독하고 있습니다..."):
                        ...
                        if err:
                            ...
                            for k, v in parsed_data.items():
                                if v is not None and k in DEFAULT_DATA:
                                    st.session_state[k] = float(v)
                                    applied_cnt += 1
                                    parsed_summary_list.append({"항목": label_map.get(k, k), "판독값": float(v)})
                            
                            if "run_no" in parsed_data and parsed_data["run_no"] is not None:
                                r_int = int(parsed_data["run_no"])
                                st.session_state.run_no = r_int
                            
                            if applied_cnt > 0:
                                st.session_state.widget_ver += 1
                                st.session_state.last_parsed_info = parsed_summary_list
                                st.rerun()
                            else:
                                st.warning("⚠️ 사진에서 인식 가능한 수치를 찾지 못했습니다. 사진이 선명한지 확인해 주세요.")

    # 방금 적용된 판독 목록 확인 안내
    if "last_parsed_info" in st.session_state and st.session_state.last_parsed_info:
        st.success(f"🎉 판독 완료! 총 **{len(st.session_state.last_parsed_info)}개** 수치가 아래 입력창에 즉시 반영되었습니다.")
        with st.expander("📋 AI가 판독하여 입력창에 주입한 수치 확인표", expanded=False):
            st.dataframe(pd.DataFrame(st.session_state.last_parsed_info), use_container_width=True)

    with st.expander("📝 이번 회차 실험 수치 입력 폼", expanded=True):
        col_in1, col_in2 = st.columns(2)
        with col_in1:
            st.markdown("#### [1. 투입 원료 및 반응 조건]")
            run_no_val = st.number_input("실험 회차 (Run No.)", min_value=1, value=int(st.session_state.run_no), step=1, key=f"inp_run_no_{v_ver}")
            st.session_state.run_no = run_no_val

            st.session_state.li2co3_mass = st.number_input("Li₂CO₃ 투입량 (g)", value=float(st.session_state.li2co3_mass), format="%.2f", key=f"inp_li2co3_mass_{v_ver}")
            st.session_state.li2co3_water = st.number_input("Li₂CO₃ 용매수 (g)", value=float(st.session_state.li2co3_water), format="%.1f", key=f"inp_li2co3_water_{v_ver}")
            st.session_state.fresh_cao_mass = st.number_input("신품 CaO 투입량 (g)", value=float(st.session_state.fresh_cao_mass), format="%.2f", key=f"inp_fresh_cao_mass_{v_ver}")
            st.session_state.recycled_cao_mass = st.number_input("재생 CaO 투입량 (g)", value=float(st.session_state.recycled_cao_mass), format="%.2f", key=f"inp_recycled_cao_mass_{v_ver}")
            st.session_state.slurry_water = st.number_input("슬러리 조제수 (g)", value=float(st.session_state.slurry_water), format="%.1f", key=f"inp_slurry_water_{v_ver}")
            st.session_state.temp_c = st.number_input("반응 온도 (℃)", value=float(st.session_state.temp_c), format="%.1f", key=f"inp_temp_c_{v_ver}")
            st.session_state.time_h = st.number_input("반응 시간 (시간)", value=float(st.session_state.time_h), format="%.1f", key=f"inp_time_h_{v_ver}")

        with col_in2:
            st.markdown("#### [2. LiOH 용액 여과 및 CaCO₃ 수세]")
            st.session_state.primary_filtrate_mass = st.number_input("LiOH 용액 무게 (g)", value=float(st.session_state.primary_filtrate_mass), format="%.1f", key=f"inp_primary_filtrate_mass_{v_ver}")
            st.session_state.primary_filtrate_sg = st.number_input("LiOH 용액 비중 (g/mL)", value=float(st.session_state.primary_filtrate_sg), format="%.3f", step=0.001, key=f"inp_primary_filtrate_sg_{v_ver}")
            
            v_primary_calc_ml = st.session_state.primary_filtrate_mass / st.session_state.primary_filtrate_sg if st.session_state.primary_filtrate_sg > 0 else 0.0
            st.info(f"🧪 **LiOH 용액 환산 부피:** `{v_primary_calc_ml:.1f} mL` (비중 {st.session_state.primary_filtrate_sg:.3f} g/mL 기준)")

            st.session_state.primary_filtrate_ph = st.number_input("LiOH 용액 pH", value=float(st.session_state.primary_filtrate_ph), format="%.2f", step=0.05, key=f"inp_primary_filtrate_ph_{v_ver}")
            
            st.session_state.wet_cake_mass = st.number_input("CaCO₃ 무게 (습중량, g)", value=float(st.session_state.wet_cake_mass), format="%.1f", key=f"inp_wet_cake_mass_{v_ver}")
            st.session_state.sample_wet = st.number_input("함수율 측정 샘플 습중량 (g)", value=float(st.session_state.sample_wet), format="%.1f", key=f"inp_sample_wet_{v_ver}")
            st.session_state.sample_dry = st.number_input("함수율 측정 샘플 건중량 (g)", value=float(st.session_state.sample_dry), format="%.1f", key=f"inp_sample_dry_{v_ver}")
            
            calc_moisture_val = (1.0 - (st.session_state.sample_dry / st.session_state.sample_wet)) * 100.0 if st.session_state.sample_wet > 0 else 0.0
            calc_dry_caco3_val = st.session_state.wet_cake_mass * (st.session_state.sample_dry / st.session_state.sample_wet) if st.session_state.sample_wet > 0 else 0.0
            st.success(f"🧱 **CaCO₃ 함수율:** `{calc_moisture_val:.2f} %` | **함수율 기준 건조 CaCO₃ 무게:** `{calc_dry_caco3_val:.1f} g`")

            st.session_state.wash_water_in = st.number_input("💧 투입된 수세수 무게 (g)", value=float(st.session_state.wash_water_in), format="%.1f", key=f"inp_wash_water_in_{v_ver}")
            
            wash_ratio_wet = (st.session_state.wash_water_in / st.session_state.wet_cake_mass) if st.session_state.wet_cake_mass > 0 else 0.0
            wash_ratio_dry = (st.session_state.wash_water_in / calc_dry_caco3_val) if calc_dry_caco3_val > 0 else 0.0
            st.caption(f"ℹ️ **수세 배수:** 습케이크 대비 `{wash_ratio_wet:.2f} 배` | 건조 CaCO₃ 대비 `{wash_ratio_dry:.2f} 배`")

            st.session_state.wash_sol_mass = st.number_input("회수된 수세액 무게 (g)", value=float(st.session_state.wash_sol_mass), format="%.1f", key=f"inp_wash_sol_mass_{v_ver}")
            st.session_state.wash_sol_sg = st.number_input("수세액 비중 (g/mL)", value=float(st.session_state.wash_sol_sg), format="%.3f", step=0.001, key=f"inp_wash_sol_sg_{v_ver}")
            
            v_wash_calc_ml = st.session_state.wash_sol_mass / st.session_state.wash_sol_sg if st.session_state.wash_sol_sg > 0 else 0.0
            st.info(f"🧪 **수세액 환산 부피:** `{v_wash_calc_ml:.1f} mL` (비중 {st.session_state.wash_sol_sg:.3f} g/mL 기준)")

            st.session_state.wash_sol_ph = st.number_input("수세액 pH", value=float(st.session_state.wash_sol_ph), format="%.2f", step=0.05, key=f"inp_wash_sol_ph_{v_ver}")
        ...
        with col_calc1:
            st.session_state.test_dry_cake = st.number_input("소성 투입 CaCO₃ 샘플 (g)", value=float(st.session_state.test_dry_cake), format="%.1f", key=f"inp_test_dry_cake_{v_ver}")
            st.session_state.calcined_cao = st.number_input("소성 후 회수된 CaO (g)", value=float(st.session_state.calcined_cao), format="%.1f", key=f"inp_calcined_cao_{v_ver}")
        with col_calc2:
            st.session_state.calc_temp = st.number_input("소성 온도 (℃)", value=float(st.session_state.calc_temp), format="%.1f", key=f"inp_calc_temp_{v_ver}")
            st.session_state.calc_time = st.number_input("소성 시간 (시간)", value=float(st.session_state.calc_time), format="%.1f", key=f"inp_calc_time_{v_ver}")
...
with main_tab2:
    ...
    with col_hdr2:
        run_no_tab2 = st.number_input("📌 분석 대상 회차 (Run No.)", min_value=1, value=int(st.session_state.run_no), step=1, key=f"inp_run_no_tab2_{v_ver}")
        if run_no_tab2 != st.session_state.run_no:
            st.session_state.run_no = run_no_tab2
            st.rerun()
    ...
    with icp_col1:
        ...
        st.session_state.icp_li_1 = st.number_input("Li 농도 (mg/L) - LiOH용액", value=float(st.session_state.icp_li_1), step=50.0, format="%.1f", key=f"inp_icp_li_1_{v_ver}")
        st.session_state.icp_ca_1 = st.number_input("Ca 농도 (mg/L) - LiOH용액", value=float(st.session_state.icp_ca_1), step=5.0, format="%.1f", key=f"inp_icp_ca_1_{v_ver}")
        st.session_state.icp_na_1 = st.number_input("Na 농도 (mg/L) - LiOH용액", value=float(st.session_state.icp_na_1), step=1.0, format="%.1f", key=f"inp_icp_na_1_{v_ver}")
        st.session_state.icp_si_1 = st.number_input("Si 농도 (mg/L) - LiOH용액", value=float(st.session_state.icp_si_1), step=0.5, format="%.1f", key=f"inp_icp_si_1_{v_ver}")
        st.session_state.icp_mg_1 = st.number_input("Mg 농도 (mg/L) - LiOH용액", value=float(st.session_state.icp_mg_1), step=0.1, format="%.1f", key=f"inp_icp_mg_1_{v_ver}")
        st.session_state.icp_k_1  = st.number_input("K 농도 (mg/L) - LiOH용액", value=float(st.session_state.icp_k_1), step=1.0, format="%.1f", key=f"inp_icp_k_1_{v_ver}")

    with icp_col2:
        st.markdown(f"#### 🔹 수세액 (부피: {v_wash_calc_ml:.1f} mL)")
        st.session_state.icp_li_w = st.number_input("Li 농도 (mg/L) - 수세액", value=float(st.session_state.icp_li_w), step=50.0, format="%.1f", key=f"inp_icp_li_w_{v_ver}")
        st.session_state.icp_ca_w = st.number_input("Ca 농도 (mg/L) - 수세액", value=float(st.session_state.icp_ca_w), step=5.0, format="%.1f", key=f"inp_icp_ca_w_{v_ver}")
        st.session_state.icp_na_w = st.number_input("Na 농도 (mg/L) - 수세액", value=float(st.session_state.icp_na_w), step=1.0, format="%.1f", key=f"inp_icp_na_w_{v_ver}")
        st.session_state.icp_si_w = st.number_input("Si 농도 (mg/L) - 수세액", value=float(st.session_state.icp_si_w), step=0.5, format="%.1f", key=f"inp_icp_si_w_{v_ver}")
        st.session_state.icp_mg_w = st.number_input("Mg 농도 (mg/L) - 수세액", value=float(st.session_state.icp_mg_w), step=0.1, format="%.1f", key=f"inp_icp_mg_w_{v_ver}")
        st.session_state.icp_k_w  = st.number_input("K 농도 (mg/L) - 수세액", value=float(st.session_state.icp_k_w), step=1.0, format="%.1f", key=f"inp_icp_k_w_{v_ver}")

    with icp_col3:
        st.markdown(f"#### 🔹 CaCO₃ (총 고형분: {est_total_dry_solids:.1f} g)")
        st.session_state.solid_li_wt = st.number_input("Li 함량 (wt%) - CaCO₃", value=float(st.session_state.solid_li_wt), step=0.01, format="%.3f", key=f"inp_solid_li_wt_{v_ver}")
        st.session_state.solid_ca_wt = st.number_input("Ca 함량 (wt%) - CaCO₃", value=float(st.session_state.solid_ca_wt), step=0.5, format="%.2f", key=f"inp_solid_ca_wt_{v_ver}")
        st.session_state.solid_na_wt = st.number_input("Na 함량 (wt%) - CaCO₃", value=float(st.session_state.solid_na_wt), step=0.005, format="%.3f", key=f"inp_solid_na_wt_{v_ver}")
        st.session_state.solid_si_wt = st.number_input("Si 함량 (wt%) - CaCO₃", value=float(st.session_state.solid_si_wt), step=0.005, format="%.3f", key=f"inp_solid_si_wt_{v_ver}")
        st.session_state.solid_mg_wt = st.number_input("Mg 함량 (wt%) - CaCO₃", value=float(st.session_state.solid_mg_wt), step=0.001, format="%.3f", key=f"inp_solid_mg_wt_{v_ver}")
        st.session_state.solid_k_wt  = st.number_input("K 함량 (wt%) - CaCO₃", value=float(st.session_state.solid_k_wt), step=0.001, format="%.3f", key=f"inp_solid_k_wt_{v_ver}")
...
"""
                    ...
                    if resp and resp.text:
                        ...
                        doe_result = json.loads(raw_json)
