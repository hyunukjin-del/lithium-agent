...
# --------------------------------------------------------------------------
# [1] 기본 세션 상태 초기화 (위젯 Key와 완벽 1:1 일치)
# --------------------------------------------------------------------------
DEFAULT_DATA = {
    "run_no": 1,
    "tab1_run_no": 1,
...
# --------------------------------------------------------------------------
# [2] Gemini 최신 모델 자동 감지 및 Vision OCR 엔진
# --------------------------------------------------------------------------
...
def parse_image_with_vision(image_bytes, doc_type="lab_note"):
    ...
    try:
        ...
        if doc_type == "lab_note":
            prompt = """당신은 탄산리튬(LC) 가성화 및 Ca-Loop 습식 제련 공정의 최고 전문 데이터 분석가입니다.
업로드된 수기 실험 일지/노트 사진에서 아래 항목들의 수치를 정확히 판독하여 JSON 형식으로만 응답해 주세요.
단위(g, mL, ℃, 시간 등)나 문자는 모두 제외하고 순수 숫자(float)만 추출해야 합니다. 
판독할 수 없거나 기재되지 않은 항목은 null로 설정하세요.

[추출 대상 키 항목 및 의미]
- run_no: 실험 회차 (Run 번호)
- li2co3_mass: 탄산리튬(Li2CO3 / LC) 투입량 (g)
- li2co3_water: Li2CO3 용매수 / 용해수 (g)
- fresh_cao_mass: 신품 생석회 / 신품 CaO 투입량 (g)
- recycled_cao_mass: 재생 생석회 / 재생 CaO 투입량 (g)
- slurry_water: 슬러리 조제수 / 소화수 (g)
- temp_c: 반응 온도 (℃)
- time_h: 반응 시간 (h / 시간)
- primary_filtrate_mass: 1차 여액 / LiOH 용액 무게 (g)
- primary_filtrate_sg: LiOH 용액 비중 (g/mL)
- primary_filtrate_ph: LiOH 용액 pH
- wet_cake_mass: 1차 습케이크 / CaCO3 무게 / 습중량 (g)
- sample_wet: 함수율 측정용 샘플 습중량 (g)
- sample_dry: 함수율 측정용 샘플 건중량 (g)
- wash_water_in: 투입된 수세수 / 세척수 무게 (g)
- wash_sol_mass: 회수된 수세액 무게 (g)
- wash_sol_sg: 수세액 비중 (g/mL)
- wash_sol_ph: 수세액 pH
- test_dry_cake: 소성 투입 CaCO3 건조케익 샘플 (g)
- calcined_cao: 소성 후 회수된 CaO 무게 (g)
- calc_temp: 소성 / 하소 온도 (℃)
- calc_time: 소성 / 하소 시간 (h / 시간)

반드시 아래 JSON 구조로만 반환하세요:
{
  "run_no": null, "li2co3_mass": null, "li2co3_water": null,
  "fresh_cao_mass": null, "recycled_cao_mass": null, "slurry_water": null,
  "temp_c": null, "time_h": null, "primary_filtrate_mass": null,
  "primary_filtrate_sg": null, "primary_filtrate_ph": null,
  "wet_cake_mass": null, "sample_wet": null, "sample_dry": null,
  "wash_water_in": null, "wash_sol_mass": null, "wash_sol_sg": null, "wash_sol_ph": null,
  "test_dry_cake": null, "calcined_cao": null, "calc_temp": null, "calc_time": null
}"""
        else:
            prompt = """이 이미지는 LiOH 용액 및 수세액(mg/L), 그리고 CaCO3 고체(wt%) 성적서입니다.
추출 가능한 수치를 아래 JSON 포맷으로 반환해 주세요. 단위는 제외하고 순수 숫자(float)만 넣어주세요.

{
  "icp_li_1": null, "icp_ca_1": null, "icp_na_1": null, "icp_si_1": null, "icp_mg_1": null, "icp_k_1": null,
  "icp_li_w": null, "icp_ca_w": null, "icp_na_w": null, "icp_si_w": null, "icp_mg_w": null, "icp_k_w": null,
  "solid_li_wt": null, "solid_ca_wt": null, "solid_na_wt": null, "solid_si_wt": null, "solid_mg_wt": null, "solid_k_wt": null
}"""
...
# --------------------------------------------------------------------------
# TAB 1: 실험 데이터 입력 및 기초 M/B 연산 (Vision 자동 입력 버그 완벽 수정)
# --------------------------------------------------------------------------
with main_tab1:
                    ...
                    with st.spinner("Gemini AI가 수기 노트를 판독하고 있습니다..."):
                        ...
                        if err:
                            ...
                            for k, v in parsed_data.items():
                                if v is not None:
                                    f_val = float(v)
                                    st.session_state[k] = f_val
                                    applied_cnt += 1
                                    parsed_summary_list.append({"항목": k, "판독값": f_val})
                            ...
                            st.success(f"🎉 판독 완료! 총 {applied_cnt}개 수치가 입력창에 즉시 반영되었습니다.")
                            with st.expander("🔍 AI 판독 항목 및 주입된 수치 확인", expanded=False):
                                st.table(pd.DataFrame(parsed_summary_list))
                            st.rerun()
    ...
    with st.expander("📝 이번 회차 실험 수치 입력 폼", expanded=True):
        ...
        with col_calc1:
            test_dry_cake = st.number_input("소성 투입 CaCO₃ 샘플 (g)", format="%.1f", key="test_dry_cake")
    ...
    # M/B 연산
    li2co3_mass = st.session_state.li2co3_mass
    li2co3_water = st.session_state.li2co3_water
    fresh_cao_mass = st.session_state.fresh_cao_mass
    recycled_cao_mass = st.session_state.recycled_cao_mass
    slurry_water = st.session_state.slurry_water
    primary_filtrate_mass = st.session_state.primary_filtrate_mass
    wet_cake_mass = st.session_state.wet_cake_mass
    wash_water_in = st.session_state.wash_water_in
    wash_sol_mass = st.session_state.wash_sol_mass
    test_dry_cake = st.session_state.test_dry_cake
    calcined_cao = st.session_state.calcined_cao
...
with main_tab2:
    ...
    wt_solid = [st.session_state.solid_li_wt, st.session_state.solid_ca_wt, st.session_state.solid_na_wt, st.session_state.solid_si_wt, st.session_state.solid_mg_wt, st.session_state.solid_k_wt]
    ...
    m4.metric("LiOH 용액 농도", f"{lioh_equiv_g_l:.2f} g/L", f"Li: {st.session_state.icp_li_1:,.1f} mg/L")
    ...
    if save_clicked:
        new_row = {
            ...
            "LiOH용액 Li농도 (mg/L)": round(st.session_state.icp_li_1, 1),
        ...
        if st.session_state.auto_email_on_save:
            ok, msg_res = send_email_report(
                ...
                li_cake_loss=li_cake_loss_pct, lioh_conc=lioh_equiv_g_l, li_1=st.session_state.icp_li_1,
                ca_1=st.session_state.icp_ca_1, na_1=st.session_state.icp_na_1, si_1=st.session_state.icp_si_1, mg_1=st.session_state.icp_mg_1, k_1=st.session_state.icp_k_1,
...
with main_tab3:
    ...
    sim_rows = []
    base_li_conc = st.session_state.icp_li_1 if 'icp_li_1' in st.session_state else 10500.0
    ...
    if "latest_doe" in st.session_state and st.session_state.latest_doe:
        ...
        if st.button("📥 이 추천 레시피를 1번 탭 입력창에 즉시 반영하기", type="primary"):
            ...
            for k in ["li2co3_mass", "li2co3_water", "fresh_cao_mass", "recycled_cao_mass", "slurry_water", "wash_water_in", "temp_c", "time_h", "calc_temp", "calc_time"]:
                if k in rec:
                    st.session_state[k] = float(rec[k])
...
배터리 소재 품질 및 양론적 관점에서 친절하고 명확하게 답변해 주세요."""
                ...
                ai_reply = f"현재 수세수 투입량은 **{st.session_state.wash_water_in:.1f} g**, 수세액 회수율 기여도는 **{li_rec_w_pct:.2f}%**입니다. (API 호출 실패: {e})"
        else:
